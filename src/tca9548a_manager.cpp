    ...
    : rclcpp::Node("tca9548a_manager", options)
{
    ...
    std::vector<int> addresses_int = this->declare_parameter<std::vector<int>>("i2c_addresses", std::vector<int>{112, 113});
    ...
    for (int address_int : addresses_int) {
        ...
        auto client = this->create_client<tca9548a::srv::SelectChannel>(service_name);
...
void Tca9548aManager::timer_callback() {
    // In this example, we'll cycle through channels 0 to 7 on the first device (client index 0).
    if (clients_.empty()) {
...
void Tca9548aManager::call_select_channel_service(size_t client_index, uint8_t channel) {
    ...
    if (!client->wait_for_service(std::chrono::seconds(1))) {
        RCLCPP_ERROR(this->get_logger(), "Service not available for client %zu.", client_index);
        return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Service request sent to client %zu for channel %d.", client_index, channel);
}
